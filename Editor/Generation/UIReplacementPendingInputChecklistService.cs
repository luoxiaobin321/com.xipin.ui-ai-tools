...
namespace Xipin.UIAITools
{
    public static class UIReplacementPendingInputChecklistService
    {
        ...
        {
            var root = Path.Combine(Path.GetTempPath(), "UIAIToolsReplacementPendingInputsContract_" + Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                ReadRows(profile);
                ExpectRowsFailure(profile, "duplicate_pending_input_row", new[]
                {
                    CsvRow("Preview", "PendingPreview", "Assets/Art/UI/AI/Demo/preview.png", "", "", "-1", "1", "ConfirmDraftPreview", "人工确认新版预览图已生成"),
                    CsvRow("Preview", "PendingPreview", "Assets/Art/UI/AI/Demo/preview.png", "", "", "-1", "1", "ConfirmDraftPreview", "人工确认新版预览图已生成")
            ...
            Debug.Log("UI replacement pending input checklist contract validation passed.");
        }
        ...
        static void ValidateNoDuplicateRows(List<Dictionary<string, string>> rows)
        {
            ...
            if (duplicate != null)
                throw new Exception($"Invalid UI replacement pending input checklist: duplicate pending input row for {duplicate.Key.InputKind} {duplicate.Key.Path}");
        }
        ...
        {
            var path = UIReportFiles.GetPath(profile.logRoot, UIReportFiles.ReplacementPendingInputs);
            File.WriteAllLines(path, new[] { UIReportFiles.ReplacementPendingInputsHeader }.Concat(rows), new UTF8Encoding(true));
        }

        static string CsvRow(string kind, string status, string path, string referencePath, string targetAtlas, string itemIndices, string count, string action, string note)
        {
            return string.Join(",", new[] { kind, status, path, referencePath, targetAtlas, itemIndices, count, action, note }.Select(Csv));
        }
        ...
        static void ExpectRowsFailure(UIAIToolsProfile profile, string name, IEnumerable<string> rows, string expectedMessage)
        {
            ...
            catch (Exception exception)
            {
                if (exception.Message.Contains(expectedMessage))
                    return;
                throw new Exception($"Unexpected UI replacement pending input checklist contract failure for {name}: {exception.Message}");
            }
            throw new Exception("UI replacement pending input checklist contract sample did not fail: " + name);
        }
    }
}

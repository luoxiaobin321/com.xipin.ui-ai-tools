...
Markdown 汇总建议列出目标 prefab、节点数量、失败项、跳过项、人工确认记录、生成后验证结果、预览图检查结果和下一步。当前宿主样例固定顶层标题为 `目标`、`状态分布` 和 `下一步`；写入、追加预览、追加深度验证和结果验证都会精确校验该结构。

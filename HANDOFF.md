...
- `Editor/Generation/UIRedesignRequestValidation.cs`：改版 request source prefab、sourcePreviewPath、outputFolder、inputImageFolder、reuseCandidateReportPath 和 referenceImagePaths 边界。
...
- `Editor/Generation/UICreationBriefTemplateService.cs`：Creation brief JSON 读取、targetFolder、参考图路径和数组项类型 gate。
- `Editor/Generation/UILayoutDraftTemplateService.cs`：Creation layout draft JSON 读取和必需字段类型 gate。
- `Editor/Generation/UICreationLayoutDryRunService.cs`：Creation layout dry-run 读取、校验、路径证据和重复检查行 gate。
...
# Run / Test
- `ValidateReplacementPlanDryRunContractBatch` -> `Logs/Verify_ReplacementPlanDryRunContract_PathBoundaries.log`，覆盖替换 dry-run 重复检查行和资源路径复验，return code 0。
...
- `ValidateRedesignPackageContractBatch` -> `Logs/Verify_RedesignPackageContract_RequestReferencePaths.log`，覆盖改版 request referenceImagePaths 路径边界复验，return code 0。
- `ValidateRedesignPackageContractBatch` -> `Logs/Verify_RedesignPackageContract_RequestInputFolder.log`，覆盖改版 request inputImageFolder 路径边界复验，return code 0。
- `ValidateRedesignPackageContractBatch` -> `Logs/Verify_RedesignPackageContract_RequestSourcePreview.log`，覆盖改版 request sourcePreviewPath 图片路径边界复验，return code 0。
- `ValidateRedesignPackageContractBatch` -> `Logs/Verify_RedesignPackageContract_RequestReuseReport.log`，覆盖改版 request reuseCandidateReportPath CSV 路径边界复验，return code 0。
...
- `ValidateUICreationBriefContractBatch` -> `Logs/Verify_UICreationBriefContract_ReferencePaths.log`，覆盖 Creation brief referenceImagePaths 路径复验，return code 0。
...
- `ValidateUICreationLayoutDryRunContractBatch` -> `Logs/Verify_UICreationLayoutDryRunContract_PathEvidence.log`，覆盖 Creation layout dry-run OK/Exists 路径证据复验，return code 0。

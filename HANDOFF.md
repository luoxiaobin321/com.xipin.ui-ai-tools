...
- `Editor/Scanning/UIScanReportRows.cs`：核心扫描报告读取和 `UIReuseIndex.csv`、`UIAssetTriageReport.csv` 重复 Path gate。
...
- `ValidateScanReportRowsContractBatch` -> `Logs/Verify_ScanReportRowsContract_DuplicateCorePathRows.log`，覆盖 `UIReuseIndex.csv` 和 `UIAssetTriageReport.csv` 重复 Path 复验，return code 0。

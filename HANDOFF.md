...
# Status
替换链路的 manifest、host apply checklist 和 host apply result 已串起前置复验；host apply result 现在要求执行计划中所有可产生结果的 action（`MoveNewAsset`、`ApplyPrefabReference`、`UpdateAtlas`、`UpdateAddress`、`VerifyAfterApply`）都有结果行，并拒绝同一计划行重复上报。`RequiresResultAction` 和 `UIReplacementPlanStatus` 已公开给宿主样例复用。Creation host generate result 会复验生成前清单，并校验目标 prefab 与当前 dry-run 组件列表。UIVipcard 真实 apply 仍按外部输入缺失预期阻断：`PendingPreview：1，PendingAsset：13，PendingAtlas：13`。

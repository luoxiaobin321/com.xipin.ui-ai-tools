...
- `Editor/Generation/UICreationHostGenerateResultService.cs`：Creation 宿主生成结果契约，结果对布局草稿校验前复验生成前清单、目标 prefab 和组件列表。
...
- 最近已验证：`ValidateUICreationHostGenerateResultContractBatch` -> `Logs/Verify_UICreationHostGenerateResultContract_ChecklistTarget.log`，exit code 0。
- 最近已验证：`ValidateUICreationHostGenerateResultContractBatch` -> `Logs/Verify_UICreationHostGenerateResultContract_ChecklistComponents.log`，exit code 0。

...
- `Editor/Scanning/UIScanReportRows.cs`：核心扫描报告读取、资产 Path 重复 gate 和 prefab 汇总重复 Prefab gate。
...
- `ValidateScanReportRowsContractBatch` -> `Logs/Verify_ScanReportRowsContract_DuplicatePrefabSummaryRows.log`，覆盖核心资产报告重复 Path 和 prefab 汇总报告重复 Prefab 复验，return code 0。

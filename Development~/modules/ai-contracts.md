...
`UIReplacementPendingInputReadinessService` 只按当前文件系统检查待补输入是否已落位，并对 Preview/NewAsset 做 PNG 可读性检查；`ValidateNoMissing` 会阻断 Missing 或 Invalid，不生成图片、不创建图集。
...
`UIReplacementHostApplyChecklistService` 用于把当前执行计划整理成宿主执行前 Markdown 清单，并提供阻断步骤 gate。它不执行资源迁移、prefab 覆盖、图集修改或 YooAsset 配置修改。

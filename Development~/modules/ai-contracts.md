...
`UIReplacementHostApplyResultService` 用于校验宿主执行器输出的 `UIReplacementHostApplyResult.csv/md`，并可从 CSV 生成结果 Markdown；它只检查精确表头、非空结果行、基础字段、状态、确认记录、失败说明、Markdown 顶层标题、状态分布和执行后复验清单，不执行资源改动。

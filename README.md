...
- 包内提供全局 AI 配置和换皮 AI 映射助手；AI 输出只作为候选 JSON 和报告，不直接写正式资源。
- 包内提供训练沉淀路由，把宿主专项经验和包内通用候选分开写到宿主工作区，避免训练时直接改包文件。
...
- AI 辅助：全局配置 OpenAI Responses API，读取效果图和候选切图生成换皮槽位映射建议。
- 训练沉淀：记录宿主专项规则和包内通用候选，包内候选后续由包维护流程提升到包仓库。
